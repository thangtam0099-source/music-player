...
router.post('/login', async (req, res) => {
  ...
  if (!user) return res.render('login', { error: 'Tài khoản không tồn tại.', settings });

  const match = await bcrypt.compare(password, user.password);
  if (!match) return res.render('login', { error: 'Mật khẩu không đúng.', settings });
...
router.get('/register', (req, res) => {
  if (req.session?.user) return res.redirect('/');
  res.render('register', { error: null, success: null, settings: getSettings() });
});
...
router.post('/register', async (req, res) => {
  ...
  const existing = db.get('users').find({ username: username.trim() }).value();
  if (existing) return fail('Username đã tồn tại.');

  const hashed = await bcrypt.hash(password, 10);
  const { nextId } = require('../database/db');
  db.get('users').push({ id: nextId('users'), username: username.trim(), password: hashed, role: 'guest' }).write();
